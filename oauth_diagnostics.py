...
def check_server_running():
    ...
    if not running_on:
        print(f"\n  ❌ Django server not found on common ports (8000, 5000, 3000, 8080)")
...
def check_supabase_config():
    ...
    if not url or not key:
        print(f"  ❌ SUPABASE_URL or SUPABASE_KEY not set in .env")
        return False
    ...
    print(f"\n  ⚠️  CRITICAL: Verify Supabase OAuth configuration manually:")
    print(f"     1. Go to: https://app.supabase.com/")
    print(f"     2. Select your project")
    print(f"     3. Go to: Authentication → Providers → Google")
...
def generate_expected_callback_urls():
    ...
    for host, subdomain, description in test_hosts:
        ...
        print(f"  • {description:40} → {callback_url}")
...
def check_django_urls():
    ...
    try:
        ...
        print(f"  ✅ auth_callback route found: {url}")
        
        url = reverse('google_login')
        print(f"  ✅ google_login route found: {url}")
...
def show_solution():
  ...
  Step 3: Copy these exact callback URLs into the "Redirect URLs" field
          ...
          For Production:
          ✅ https://nexassearch.com/auth/callback/
          ✅ https://beddingsavenue.nexassearch.com/auth/callback/
          ✅ https://*.nexassearch.com/auth/callback/
...
def run_diagnostics():
    ...
    if not server_running:
        print(f"\n⚠️  REQUIRED: Start the Django dev server before testing OAuth")
        return False
    ...
    if server_running and accessible:
        print("""
  ✅ Your server is running and callback URL is accessible
  ❌ If you're still getting "localhost refused", the issue is:
  
     → Callback URL NOT registered in Supabase OAuth settings
...
""")
        ...
        print("""
  ⚠️  Django server is running but callback URL not accessible
