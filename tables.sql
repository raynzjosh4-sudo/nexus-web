...
CREATE TABLE public.item_ai_tags (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  item_id uuid NOT NULL,
  item_type character varying NOT NULL,
  detected_category character varying,
  detected_urgency character varying,
  detected_quality_tier character varying,
  sentiment_score double precision,
  has_quality_images boolean DEFAULT false,
  estimated_authenticity_score double precision,
  is_likely_fraud boolean DEFAULT false,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT item_ai_tags_pkey PRIMARY KEY (id)
);
CREATE TABLE public.item_rank_scores (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  item_id uuid NOT NULL,
  item_type character varying NOT NULL,
  user_id uuid,
  logistical_feasibility_score double precision DEFAULT 0,
  distance_km double precision,
  owner_trust_score double precision DEFAULT 0.5,
  engagement_score double precision DEFAULT 0,
  relevance_score double precision DEFAULT 0,
  final_rank_score double precision DEFAULT 0,
  calculated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT item_rank_scores_pkey PRIMARY KEY (id),
  CONSTRAINT item_rank_scores_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.nexususers(id)
);
...
CREATE TABLE public.page_pins (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  page_name text NOT NULL UNIQUE CHECK (page_name <> ''::text),
  pin_hash text NOT NULL CHECK (pin_hash <> ''::text),
  created_at timestamp without time zone DEFAULT now(),
  updated_at timestamp without time zone DEFAULT now(),
  created_by uuid,
  is_active boolean DEFAULT true,
  CONSTRAINT page_pins_pkey PRIMARY KEY (id),
  CONSTRAINT page_pins_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id)
);
CREATE TABLE public.pin_access_log (
  ...
  created_at timestamp without time zone DEFAULT now(),
  CONSTRAINT pin_access_log_pkey PRIMARY KEY (id),
  CONSTRAINT pin_access_log_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
CREATE TABLE public.pin_access_permissions (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  page_name text NOT NULL CHECK (page_name <> ''::text),
  user_id uuid NOT NULL,
  granted_by uuid NOT NULL,
  granted_at timestamp without time zone DEFAULT now(),
  expires_at timestamp without time zone,
  is_active boolean DEFAULT true,
  CONSTRAINT pin_access_permissions_pkey PRIMARY KEY (id),
  CONSTRAINT pin_access_permissions_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id),
  CONSTRAINT pin_access_permissions_granted_by_fkey FOREIGN KEY (granted_by) REFERENCES auth.users(id)
);
...
CREATE TABLE public.user_engagements (
  ...
  item_id uuid NOT NULL,
  item_type character varying NOT NULL,
  engagement_type character varying NOT NULL,
  engagement_duration_seconds integer,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT user_engagements_pkey PRIMARY KEY (id),
  CONSTRAINT user_engagements_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.nexususers(id)
);
...
CREATE TABLE public.user_preferences (
  ...
  CONSTRAINT user_preferences_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.nexususers(id),
  CONSTRAINT user_preferences_category_id_fkey FOREIGN KEY (category_id) REFERENCES public.categories(id)
);
...
CREATE TABLE public.user_trust_scores (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL UNIQUE,
  purchase_completion_rate double precision DEFAULT 0,
  payment_on_time_rate double precision DEFAULT 0,
  positive_buyer_reviews integer DEFAULT 0,
  negative_buyer_reviews integer DEFAULT 0,
  item_as_described_rate double precision DEFAULT 0,
  shipping_on_time_rate double precision DEFAULT 0,
  positive_seller_reviews integer DEFAULT 0,
  negative_seller_reviews integer DEFAULT 0,
  trade_completion_rate double precision DEFAULT 0,
  trade_honor_rate double precision DEFAULT 0,
  overall_trust_score double precision DEFAULT 0.5,
  last_updated timestamp with time zone DEFAULT now(),
  CONSTRAINT user_trust_scores_pkey PRIMARY KEY (id),
  CONSTRAINT user_trust_scores_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.nexususers(id)
);
